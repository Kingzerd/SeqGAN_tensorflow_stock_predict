...
def add_layer(input_data, data_input_size, data_output_size):
    w = tf.Variable(tf.random_normal([data_input_size, data_output_size]))
    bias = tf.Variable(tf.constant(0.1, shape=[data_output_size, ]))
...
def lstm(X):
    ...
    # input_rnn = tf.nn.tanh(tf.matmul(input_rnn, w_in3) + b_in3)
    # input_rnn = add_layer(input_rnn, rnn_unit, rnn_unit)
...
# batch_size原为80
def train_lstm(batch_size=60, time_step=data_time_step, train_begin=data_train_begin_2,
               train_end=data_train_end):  # time_step原为15
    ...
    with tf.Session() as sess:
        try:
            module_file = tf.train.latest_checkpoint('./model_save/model_multi/')
            print(module_file)
        ...
        for i in range(1501):
            ...
            if i % 1000 == 0:
                print("保存模型：", saver.save(sess, './model_save/model_multi/stock2.model', global_step=i))
            print(i, loss_)
