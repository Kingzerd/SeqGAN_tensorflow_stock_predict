...
class Generator(object):
    def __init__(self, config):
        ...
        Args:
           ...
           start_token: special token used to represent start of time series
        ...
        with tf.variable_scope("teller"):
            ...
            with tf.variable_scope("lstm"):
                for j in range(self.sequence_length):
                    ...
                    # calculate loss
                    pretrained_loss_t = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.input_seqs_pre[:, j],
                                                                                       logits=logits)
    ...
    def build_sample_network(self):
        ...
        with tf.variable_scope("teller"):
            ...
            self.sample_word_list_reshape = tf.transpose(tf.squeeze(tf.stack(self.sample_word_list)),
                                                         perm=[1, 0])  # batch_size * sequene_length
