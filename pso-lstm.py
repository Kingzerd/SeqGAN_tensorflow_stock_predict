...
def train_lstm(batch_size=80, time_step=15, train_begin=data_train_begin_2, train_end=data_train_end):
    ...
    with tf.Session() as sess:
        ...
        for i in range(11):
            ...
            if i % 200 == 0:
                print("保存模型：", saver.save(sess, './model_save/model/stock2.model', global_step=i))
                # writer = tf.summary.FileWriter("F:\PythonPrograms\stock_predict\model_save\model", sess.graph)
            print(i, loss_)
...
def prediction(time_step=20):
    ...
    with tf.Session() as sess:
        # 参数恢复
        module_file = tf.train.latest_checkpoint('./model_save/model/')
        saver.restore(sess, module_file)
        test_predict = []
        ...
        print("偏差：", acc)
        writer = tf.summary.FileWriter("F:\PythonPrograms\SeqGAN_tensorflow\model_save\model", sess.graph)
        ...
        #     r2 = random.random()
        #     if r1>=0.5:
        #         test_random.append(test_y[b]+1*r2)
        #     else:
        #         test_random.append(test_y[b] - 1 * r2)
        ...
        # plt.plot(list(range(len(test_random))), test_random, color='b')
        plt.plot(list(range(len(test_y))), test_y, color='r')
        plt.legend(['predict', 'truth'])
        plt.show()

        file = "save/generator_sample.txt"
        with open(file, 'a+') as f:
            count = 1
            length = len(test_predict)
            while (20 * count < length):
                buffer = ' '.join([str(int(x)) for x in test_predict[20 * (count - 1):20 * count]]) + '\n'
                print(buffer)
                f.write(buffer)
                count += 1
